...
| 3 | `legacy-behavior-characterizer` | 基准文件和特征测试 | `feature-extension-planner`、`refactor-slicer` |
| 4 | `dirty-data-case-designer` | 脏数据样例和预期行为 | `feature-extension-planner`、`regression-validation-planner` |
| 5 | `feature-extension-planner` | 多股票、行业输出、停牌处理和兼容扩展方案 | `refactor-slicer` |
| 6 | `refactor-slicer` | 有序安全实现切片 | `regression-validation-planner`、实现与审查 |
| 7 | `regression-validation-planner` | 回归验证矩阵 | `compatibility-reviewer`、`gray-release-planner` |
| 8 | `compatibility-reviewer` | 兼容性问题和必要缓解措施 | 发布准备 |
| 9 | `gray-release-planner` | 灰度发布和回滚计划 | 交付 |
| 10 | `ai-usage-recorder` | AI_USAGE.md 草案 | 最终交付 |
...
- 扩展阶段必须显式说明多股票批处理、行业输出、停牌缺失数据处理和旧输出兼容策略。
- 验证阶段必须显式覆盖单元测试、基准文件回归、结构契约、下游冒烟、新旧差异和性能验证。
